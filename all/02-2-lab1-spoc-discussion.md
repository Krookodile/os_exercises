...
> 
  先实现一个可显示字符串的bootloader，再实现一个可加载ELF格式文件的bootloader
...
> 
  建立一个表A（表示为一个数组），每一个表项都是一个系统调用，另外新建一个数组B，存储系统调用的次数，与A对应，  B[i]表示系统调用到A[i]的时候系统调用的次数，每一次系统调用存入A之前对B把之前的调用次数加一，写入相应的地址，   如此就可以实现查询系统调用的次数了。
